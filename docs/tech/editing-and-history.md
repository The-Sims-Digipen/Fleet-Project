...
Selection, catalog creation/deletion, reset actions, panel collapse, and history navigation finish pending edits. Undo includes object properties, creation/deletion, lighting, and resets; selection, model loading, camera movement, and panel expansion are excluded. Selection is cleared if its object is removed by any document change, including undo/redo. Restoring an object through history does not reselect it. New edits discard redo, and no-op edits add no history. Ctrl/Cmd+Z undoes, Ctrl+Y or Ctrl/Cmd+Shift+Z redoes; focused form fields retain native shortcuts.
