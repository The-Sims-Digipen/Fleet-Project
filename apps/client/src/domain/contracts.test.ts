...
import { sim01Expected, sim01Project, sim01Scenario } from "./m1Fixture";
...
describe("M1 integration contracts", () => {
  ...
  it("keeps the shared SIM01 fixture reference-complete", () => {
    const input: SimulationInput = { project: sim01Project, scenario: sim01Scenario };
    const presetIds = new Set(input.project.vehiclePresets.map((preset) => preset.id));
    const vehicleIds = new Set(input.project.fleetVehicles.map((vehicle) => vehicle.id));

    for (const vehicle of input.project.fleetVehicles) expect(presetIds.has(vehicle.currentPresetId)).toBe(true);
