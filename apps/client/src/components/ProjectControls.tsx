...
export function ProjectControls() {
  ...
  return <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-3 gap-y-1" aria-label="Project">
    ...
    <div className="ml-auto flex gap-2 self-end">
      ...
      <button type="button" aria-label={saveStatus.state === "error" ? "Retry saving project" : "Save project"} className="min-h-9 rounded-lg bg-accent px-3 text-xs font-bold text-accent-ink disabled:cursor-default disabled:opacity-40" disabled={saveStatus.state === "saving"}
